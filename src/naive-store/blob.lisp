...
(defun read-blob (blob-ref-values)
  ...
  (let ((*read-eval* nil)
    ...
    (let ((path (pathname (getx blob-ref-values :location))))
      (make-blob
       :file-type (or (getx blob-ref-values :file-type) (pathname-type path))
       :file-ext (or (getx blob-ref-values :file-ext) (pathname-type path))
